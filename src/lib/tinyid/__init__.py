...
class TinyIDGenerator(object):
    ...
    def generate_tinyid(self):
        "Generates a unique tinyid, returning the actual tinyid string."
        
        def __get_or_insert(trial_id):
            key = db.Key.from_path(TinyID.kind(), trial_id, namespace='')
            existing_tinyid = db.get(key)
            if existing_tinyid:
                raise Exception()
        ...
        while i < self.max_trials:
            ...
            try:
                insert_result = db.run_in_transaction(__get_or_insert, trial_id)
                break
            except Exception:
